(ns lucid.query.common-test
  (:use [hara.test :exclude [any]])
  (:require [lucid.query.common :refer :all]))

^{:refer lucid.query.common/any :added "1.2"}
(fact "returns true for any value"
  (any nil) => true
  (any '_) => true)

^{:refer lucid.query.common/none :added "1.2"}
(fact "returns false for any value"
  (none nil) => false	
  (none '_) => false)

^{:refer lucid.query.common/expand-meta :added "1.2"}
...
^{:refer lucid.query.common/cursor? :added "1.2"}
(fact "checks if element is `|`"
  (cursor? '|) => true
  (cursor? '_) => false)

^{:refer lucid.query.common/insertion? :added "1.2"}
(fact "checks if element has an insert meta"
  (insertion? '^:+ a) => true
  (insertion? 'a) => false)

^{:refer lucid.query.common/deletion? :added "1.2"}
(fact "checks if element has a delete meta"
  (deletion? '^:- a) => true
  (deletion? 'a) => false)

^{:refer lucid.query.common/remove-items :added "1.2"}
...
^{:refer lucid.query.common/prepare-deletion :added "1.2"}
...
^{:refer lucid.query.common/prepare-insertion :added "1.2"}
(fact "removes extraneous symbols for deletion walk"
...
^{:refer lucid.query.common/find-index :added "1.2"}
(fact "returns the index of the first occurrence"
  (find-index #(= 2 %) '(1 2 3 4))
  => 1)

^{:refer lucid.query.common/finto :added "1.2"}
